        ...
        if ((address == "" || address == null) && !address) {
            ...
            $.ajax({
                ...
                success: function (xml) {
                    ...
                    var timeSetting = $(xml).find('timeSetting'); //获得是否有时间设定
                    htmlWriter += "<h1>专辑标题：<a target='_blank' href='/subject/" + address + "/index.htm'>" + subjectTitle + "</a><i><a target='_blank' href='/subject/" + address + "/scripts/data.xml'>查看线路源数据xml</a></i><i>(线路修改页面,请使用最新版Chrome浏览器)</i>v4.2.1 更新日期20131125</h1><p><b>输入框转义符：</b><b>1.换行符，回车：</b>[br]=&lt;br/\&gt;;；<b>2.加粗：</b>[b]例子[/b]=&lt;b&gt;<b>例子</b>&lt;/b&gt;;；<b>3.字号大小(未支持)：</b>[fs=12]12号字体[/fs]=&lt;font style=\"font-size:12px\"&gt;<font style='font-size:12px'>12号字体</font>&lt;/font&gt;;；可嵌套使用。<br/><b>升级内容:</b>1.xml文件检查；2.全选、反选功能</p><div id='create' class='create'><button id='createBtn' class='createBtn'>生成静态页面</button><div id='upLoader' class='upLoader'><div class='borderBox'>HTML文件上传区<p>请拖动文件到此处</p></div></div>";
                    if (timeSetting.length) {
                        $.each(timeSetting.children("time"), function (key,value) {
                            htmlWriter += '<div onClick="SetTimer()" id="" class="upLoader timeSettingZoom"><div class="borderBox">时间ID：' + key + '<p>点击查看详情</p></div></div>';
                        });
                        htmlWriter += "</div>";
                    }
                    //填写表格头部
                    htmlWriter += '<div class="static"><dl id="tableHead" class="tableHead"><dt>编号</dt><dt>上架</dt><dt>售罄 <a href="javascript:void(0);" id="allSelected" onclick="allSelected">全选</a><a id="deSelected" href="javascript:void(0);">反选</a></dt>';
                    htmlWriter += '<dt>标题</dt><dt>价格</dt><dt class="last">链接</dt></dl></div>'
                    ...
                    $(xml).find("line").each(function (i) {
                        ...
                        //                            var timeStart = timeSetting.children("time[id='" + timeId + "']").text(); //获得时间设定
                        //							var childCount=timeSetting.first().children();
                        //                            if (childCount.length>1)
                        //                            timeHTML += "<td rowspan='2' width='" + tdWidth[3] + "'><input class='soldout checkType editZoom' type='checkbox' value='N' edit-Line='" + i + "' edit-Index='1' /></td>";
                        ...
                        var remarkHTML = new String('');
                        var timeSub = '';
                        if (timeSetting.length) {
                            var timeSettingId = $(this).find('timeid').text();
                            if (ishasRemarks) {
                                timeSub = "时间ID：" + timeSettingId;
                                console.log(remarkHTML);
                            } else {
                                remarkHTML += '<tr><td colspan="6">时间ID：' + timeSettingId + '</td></tr>'
                            }
                        }
                        if (ishasRemarks) {
                            remarkHTML += "<tr><td></td><td></td><td>" + timeSub + "</td><td colspan='3'>"; //初始化读取备注
